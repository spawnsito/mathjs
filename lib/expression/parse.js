 ...
 *
 *     parse(expr, math)
 *     parse(expr, math, options)
 *     parse([expr1, expr2, expr3, ...], math)
 *     parse([expr1, expr2, expr3, ...], math, options)
 *
 * Example:
 *
 ...
 * @param {Object} math     The math namespace, must contain functions like
 *                          add, subtract, ...
 ...
 * @throws {Error}
 */
function parse (expr, math, options) {
  if (arguments.length < 1 || arguments.length > 3) {
    throw new ArgumentsError('parse', arguments.length, 1, 3);
  }
...
function parseBlock () {
  ...
  if (token == '') {
    // empty expression
    return new ConstantNode(_math, 'undefined', 'undefined');
  }
  ...
  if (blocks.length > 0) {
    return new BlockNode(_math, blocks);
  }
  else {
    return node;
  }
...
function parseAssignment () {
  ...
  if (token == '=') {
    if (node instanceof SymbolNode) {
      ...
      return new AssignmentNode(_math, name, expr);
    }
    else if (node instanceof IndexNode) {
      // parse a matrix subset assignment like 'A[1,2] = 4'
      getTokenSkipNewline();
      expr = parseAssignment();
      return new UpdateNode(_math, node, expr);
    }
      ...
      if (valid) {
        getTokenSkipNewline();
        expr = parseAssignment();
        return new FunctionAssignmentNode(_math, name, args, expr);
      }
    }
...
function parseConditional () {
  ...
  while (token == '?') {
    ...
    node = new ConditionalNode(_math, condition, trueExpr, falseExpr);
...
function parseLogicalOr() {
  ...
  while (token == 'or') {
    getTokenSkipNewline();
    node = new OperatorNode(_math, 'or', 'or', [node, parseLogicalXor()]);
  }
...
function parseLogicalXor() {
  ...
  while (token == 'xor') {
    getTokenSkipNewline();
    node = new OperatorNode(_math, 'xor', 'xor', [node, parseLogicalAnd()]);
  }
...
function parseLogicalAnd() {
  ...
  while (token == 'and') {
    getTokenSkipNewline();
    node = new OperatorNode(_math, 'and', 'and', [node, parseBitwiseOr()]);
  }
...
function parseBitwiseOr() {
  ...
  while (token == '|') {
    getTokenSkipNewline();
    node = new OperatorNode(_math, '|', 'bitOr', [node, parseBitwiseXor()]);
  }
...
function parseBitwiseXor() {
  ...
  while (token == '^|') {
    getTokenSkipNewline();
    node = new OperatorNode(_math, '^|', 'bitXor', [node, parseBitwiseAnd()]);
  }
...
function parseBitwiseAnd () {
  ...
  while (token == '&') {
    getTokenSkipNewline();
    node = new OperatorNode(_math, '&', 'bitAnd', [node, parseRelational()]);
  }
...
function parseRelational () {
  ...
  while (token in operators) {
    ...
    node = new OperatorNode(_math, name, fn, params);
  }
...
function parseRange () {
  ...
  if (token == ':') {
    ...
    node = new ConstantNode(_math, '1', 'number');
  }
  else {
    // explicit start
  ...
  if (token == ':' && (conditional_level !== nesting_level)) {
    ...
    while (token == ':' && params.length < 3) {
      ...
      if (token == ')' || token == ']' || token == ',' || token == '') {
        // implicit end
        params.push(new SymbolNode(_math, 'end'));
      }
      else {
        // explicit end
    ...
    if (params.length == 3) {
      ...
      node = new RangeNode(_math, params[0], params[2], params[1]); // start, end, step
    }
    else { // length == 2
      // params = [start, end]
      node = new RangeNode(_math, params[0], params[1]); // start, end
    }
  }
...
function parseMultiplyDivide () {
  ...
  if ((token_type == TOKENTYPE.SYMBOL) ||
    ...
    node = new OperatorNode(_math, '*', 'multiply', [node, parseMultiplyDivide()]);
  }
...
function parseUnary () {
  ...
  if (fn) {
    ...
    return new OperatorNode(_math, name, fn, params);
  }
...
function parseCustomNodes () {
  ...
  if (token_type == TOKENTYPE.SYMBOL && extra_nodes[token]) {
    ...
    return new handler(_math, params);
  }
...
function parseFunctions (name) {
  ...
  if (token == '(') {
    ...
    return new FunctionNode(_math, name, params);
  }

  return new SymbolNode(_math, name);
}
...
function parseIndex (node) {
  ...
  while (token == '[') {
    ...
    node = new IndexNode(_math, node, params);
  }
...
function parseString () {
  ...
  if (token == '"') {
    ...
    // create constant
    node = new ConstantNode(_math, str, 'string');
  ...
  if (token == '[') {
    ...
    if (token != ']') {
      ...
      if (token == ';') {
        ...
        array = new ArrayNode(_math, params);
      }
      else {
...
function parseRow () {
  ...
  return new ArrayNode(_math, params);
}
...
function parseNumber () {
  ...
  if (token_type == TOKENTYPE.NUMBER) {
    ...
    return new ConstantNode(_math, number, 'number');
  }
